...
type intervalDTO struct {
	...
	End   *int64  `json:"end"`
	// RequiredHits is optional; when absent the interval keeps the original
	// single-hit semantics. When present it must be 1, 2 or 3.
...
type responseFail struct {
	...
	FailedMissing   int         `json:"failed_missing"`
}
	...
	for _, iv := range raw {
		...
		if iv.RequiredHits != nil {
			if *iv.RequiredHits < 1 || *iv.RequiredHits > 3 {
				return nil, nil, errors.New("requiredHits must be between 1 and 3")
			}
			hits = *iv.RequiredHits
		}
		intervals = append(intervals, planner.Interval{ID: id, Start: *iv.Start, End: *iv.End, RequiredHits: hits})
	}
	...
	mux.HandleFunc("/sample-plan", func(w http.ResponseWriter, r *http.Request) {
		...
		if res.Status == "NO_SAMPLE_POINT" {
			writeJSON(w, http.StatusOK, responseFail{
				...
				FailedMissing:   res.Fail.Missing,
			})
			return
		}
		...
		for _, h := range res.Hits {
			hits = append(hits, intervalHitsJSON{IntervalID: h.IntervalID, Batches: h.Batches})
		}
